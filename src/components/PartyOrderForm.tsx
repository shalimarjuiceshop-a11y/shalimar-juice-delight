...
import { PartyPopper, Send, User, Phone, MapPin, Users } from "lucide-react";
import { z } from "zod";
...
const schema = z.object({
  ...
  quantity: z.string().trim().min(1, "Select number of guests"),
});
...
const PartyOrderForm = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const message = `🎉 *Party Order Inquiry*%0A%0A👤 *Name:* ${encodeURIComponent(form.name)}%0A📱 *Mobile:* ${encodeURIComponent(form.mobile)}%0A📍 *City:* ${encodeURIComponent(form.city)}%0A👥 *Guests:* ${encodeURIComponent(form.quantity)} people%0A%0A_Sent from Shalimar Juice Website_`;

    window.open(`https://wa.me/${WHATSAPP_NUMBER}?text=${message}`, "_blank");
    toast.success("Redirecting to WhatsApp!");
    setForm({ name: "", mobile: "", city: "", quantity: "" });
  };
  ...
  return (
    ...
    >
      ...
      <motion.div variants={fadeUp} className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h3 className="font-display text-lg font-bold text-foreground">Party Order</h3>
          <p className="font-body text-xs text-muted-foreground">Book fresh juice for your event</p>
        </div>
      </motion.div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {/* Guests */}
        <motion.div variants={fadeUp} className="relative">
          <Users size={16} className="absolute left-3.5 top-3.5 text-muted-foreground/50 z-10" />
          <select
            value={form.quantity}
            onChange={(e) => setForm({ ...form, quantity: e.target.value })}
            className={`${inputClass("quantity")} appearance-none cursor-pointer`}
          >
          ...
          {errors.quantity && <p className="text-xs text-destructive mt-1 ml-1">{errors.quantity}</p>}
        </motion.div>
